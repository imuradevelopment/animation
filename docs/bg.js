class Line {
  ...
  renderStyle(style, ctx) {
    if (style === "glitches") {
      ctx.lineDashOffset =
        this.line.mag + this.offSet;
      ctx.setLineDash([
        this.size ** 1.5,
        (this.line.mag / this.length) *
          this.size ** 2,
      ]);
      this.offSet += 20;
  ...
  mutatePath() {
    let lineFragment = this.line.path
      .split(" ")
      .slice(1);
    let generator = this.generators();
    lineFragment[
      (Math.random() * lineFragment.length) | 0
    ] =
      generator[
        (Math.random() * generator.length) | 0
      ].line;
    this.line.path = `${
    ...
    function generateLines(amount) {
      ...
      let styles = [
        ...
        },
        {
          size: 1.25,
          style: "pattern",
          color: { h: 120, s: 100, l: 70, a: 0.5 },
        },
        {
          size: 2.5,
          style: "pattern",
          color: { h: 150, s: 100, l: 50, a: 0.3 },
        },
        {
          size: 5,
          style: "pattern",
          color: { h: 90, s: 100, l: 50, a: 0.5 },
        },
        {
          size: 10,
          style: "pattern",
      ...
      for (let i = 0; i < amount; i += 1) {
        let style =
          styles[
            // ゼロ付近の割合を多くする乱数を生成し,配列をランダムの抽出する
            (Math.random() ** 2 * styles.length) | 0
          ];
        lines.push(
          new Line(
